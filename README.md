...
- 自选池预警（预警中心）：为最多 3 个自选股/自选基金配置涨幅、最新价、单位净值、净值单日涨跌与回撤类条件（支持 1 个条件或 2–4 个条件 AND/OR 组合）；交易日每 30 分钟扫描一次，命中后写入事件流并可按需发送摘要邮件，同一规则 12 小时冷却且非交易时段不触发。
...
| `SMTP_HOST` | 可选 | SMTP 服务器地址；未配置时预警仅页面展示，不发送邮件 |
| `SMTP_PORT` | 可选 | SMTP 端口，默认 `465`（465 使用 SSL，其它端口使用 STARTTLS） |
| `SMTP_USER` | 可选 | SMTP 账号 |
| `SMTP_PASS` | 可选 | SMTP 授权码或密码 |

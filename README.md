...
| 我的持仓 | 个股与基金的持仓收益、当日盈亏、权重与行业分布；数据库不可用时落本地文件 |
...
- 未配置 `DEEPSEEK_API_KEY` 或模型输出缺少具体行情数据时，AI 分析回退为包含最新价、K 线与指标的教学式报告；未配置 `TAVILY_API_KEY` 时使用本地演示资讯。
...
| 端到端 | `corepack pnpm build && corepack pnpm test:e2e` | Playwright，自动拉起 `next start`（端口 3100），覆盖首页外壳、工作台切换、自选股、背景设置与执行轨迹 |
| 持续集成 | push / PR 自动触发 | `install → typecheck → lint → test → build`，侧车仅做 Python 语法检查 |

- 端到端用例的数据隔离：`DATA_ROOT` 指向临时目录，数据库、SMTP 与外部密钥一律置空，跑的是无密钥降级路径，不会读写本机 `.data`。

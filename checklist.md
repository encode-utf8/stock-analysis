...
- 本机正在运行的行情侧车仍是改动前的进程（上限 240），需重启侧车后长区间回测才能取到完整样本
...
- [ ] 侧车新增 `/quotes` 批量接口：上限 50 个代码，非法输入返回 400，单代码缺失不整批失败
- [ ] `/quote` 单代码接口保持不变（向后兼容）
- [ ] 新增 `GET /api/stream/quotes` SSE：事件类型含 snapshot / alert / status / heartbeat
- [ ] 服务端轮询器为单例，多订阅者合并去重后只发起一次批量拉取
- [ ] 交易时段按 `QUOTE_STREAM_INTERVAL_MS`（默认 5000，下限 3000）推送，非交易时段降频到 60 秒并标注休市
- [ ] 上游连续失败时指数退避降频，连接不中断并标注降级来源
- [ ] 前端实时开关默认关闭，开启后才建连；页面隐藏自动降频
- [ ] 自选池与持仓面板展示实时价并标注「实时」与更新时间
- [ ] 连接状态指示灯正确反映已连接/降级/断开，断线自动重连
- [ ] 预警复用既有 `evaluateAlertRule` 与冷却逻辑，命中后事件流与站内通知立即可见
- [ ] 浏览器通知需用户授权，被拒时降级为站内 toast，不阻塞功能
- [ ] 提示音可开关，默认关闭
- [ ] 30 分钟 `ALERT_CRON` 保留为兜底，邮件通道行为不变
- [ ] `ALERT_MAX_TARGETS` 默认由 3 放宽到 10，仍可配置
- [ ] `corepack pnpm test`、`typecheck`、`lint`、`build` 全部通过
- [ ] 代码注释与提交信息为中文，未提交真实密钥与临时文件

...
## README 精简工程化改写（2026-09-20，开发完成，自测通过，待用户验收）

需求来源：`README.md` 原为 303 行，章节分散（当前能力、环境要求、启动参数、停止服务、手动开发启动、环境变量、定时任务守护、数据一致性清理、持续集成、端到端测试各自成节）且以长段落罗列为主，信息密度低、与主流开源项目写法差异较大；用户要求「精简工程化压缩表达，符合行业规范以及 GitHub 流行表达」。

- 关联产出：`README.md`（纯文档改写，无代码变更）
- 分支：`feature/agent-trace`（沿用当前分支）

### 任务目标与范围

- 目标：在不丢失信息的前提下压缩篇幅，向 GitHub 主流 README 结构收敛——标题与摘要 → 徽章 → 功能 → 技术栈 → 快速开始 → 常用命令 → 配置 → 数据与降级 → 定时任务 → 测试与 CI → 健康检查 → 项目结构 → 文档索引 → 免责声明。
- 范围：仅重写 `README.md`；原分散章节合并进对应新章节，长段落改为表格呈现。
- 非目标：不改代码、脚本、配置与其它文档，不新增或删减系统能力描述，不执行 `git commit` / `merge` / `push`。

### 验收项

- [x] 篇幅：303 行 → 170 行（约 -44%），`git diff --stat README.md` 为 120 insertions(+) / 253 deletions(-)
- [x] 结构：标准十四节，新增「项目结构」与「文档索引」
- [x] 信息完整：11 项能力（含本期「执行轨迹」）、环境要求、启动参数、停止方式、手工启动、降级策略、定时任务、CI 与 e2e、健康检查与日报补生成 / 删除接口全部保留
- [x] 表格化：功能 11 行、技术栈 4 行、常用命令 7 行、配置 7 组、测试与 CI 3 行、文档索引 7 行
- [x] 配置压缩：原「环境变量」39 行逐项表改为 7 组归纳，cron / 调度 / 实时行情类变量指向 `.env.example` 注释
- [x] 徽章：新增 CI 状态徽章（指向本项目 `ci.yml`）与 Node / pnpm / Next.js / Python 徽章
- [x] 引用有效：README 反引号内 23 个本地路径与 9 个 `corepack pnpm` 脚本名逐一核对存在
- [x] 排版规范：中文与拉丁字母 / 数字间加空格、全角标点、UTF-8 无 BOM、CRLF 一致、代码围栏成对（10 个）
- [x] 变更隔离：工作区除既有 Agent 轨迹改动外，仅 `README.md` 与 `checklist.md` 变化

### 验证方式

```powershell
git diff --stat README.md
Select-String -Path README.md -Pattern '^#{1,3} ' -Encoding utf8   # 章节结构
# 反引号内本地路径与 pnpm 脚本名存在性核对
```
...
- [x] 分组划分：个股「当前标的」8 项 + 「持仓与全局工具」6 项；基金 8 + 8，无重复无遗漏
...
- 分类以「面板是否依赖当前标的代码」为准：自带代码输入的策略回测、基金对比 / 组合 / 定投 / 风格 / 行业资讯归入工具组，行为保持不变。

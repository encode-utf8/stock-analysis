...
## 10. v0.4 修订：面板绑定工作台、自选池实时同步与估值源扩展（2026-09-10）

### 10.1 问题

1. 左侧自选池新增/删除后，预警面板的下拉选项要刷新整页才更新。
2. 预警面板自带「股票/基金」切换按钮，与工作台已有的切换重复，且容易在基金工作台误建股票规则。
3. 场外估值只有东财估值排行一个源（覆盖有限、偶发 SSL 中断），需要补充免费源。

### 10.2 免费估值源复核（2026-09-10 实测）

| 源 | 接口 | 结论 |
| --- | --- | --- |
| 新浪财经盘中估值 | `hq.sinajs.cn/list=fu_{code},f_{code}`（需 `Referer: https://finance.sina.com.cn`，GBK） | ✅ 首选。单只按需、无鉴权，一次请求同时拿到估算净值、估算涨跌幅、最近公布净值及净值日期 |
| 东财估值排行静态页 | AkShare `fund_value_estimation_em(symbol="指数型")` | ✅ 兜底。实测 681 只，盘后仍保留当日快照，偶发 SSL 中断 |
| 东财估值排行 API | `api.fund.eastmoney.com/FundGuZhi/GetFundGZList` | ⚠️ 盘中可用，盘后返回「暂无数据」 |
| 东财移动端 | `fundmobapi.eastmoney.com/FundMNewApi/FundMNFInfo?Fcodes=` | ⚠️ 净值可用，`GSZ/GSZZL` 仅盘中填充 |
...
| 自建持仓穿透 | 季报前十大持仓 + 实时行情加权 | 可行但成本高、依赖季报披露，本期不做 |

交叉验证：2026-09-10 快照，000008 新浪估算 2.2789 / -0.6042%、东财估算 2.2779 / -0.65%。

### 10.3 设计结论

- 场外估值链路：**新浪估值 → 东财估值排行 → 确定性降级**。新浪源加两道守门，避免脏数据报假警：
  - 估算日期必须等于当日（北京时间），否则视为旧快照丢弃；
  - `估算净值 / 昨日单位净值 - 1` 必须与接口给出的估算涨跌幅吻合（偏差超过 0.5 个百分点即丢弃）。
- 新增来源标识 `sina`，前端标注「新浪财经盘中估值」。
- 预警面板去掉股票/基金切换按钮，改为由所在工作台传入 `target`（股票工作台 → stock，基金工作台 → fund）。
- 新增 `src/lib/watchlist-bus.ts` 事件总线：自选股/自选基金增删后广播，预警面板订阅后只重拉对应自选池，实现无刷新同步。

### 10.4 遗留

- 部分基金新浪无估值（如 000187 实测返回空），此时回退东财估值排行；两个源都没有数据时按「缺少指标观测值」跳过。
- 估值仍为估算口径，与官方净值存在偏差，面板保留「估算值，非官方净值」提示。
...
- 口径更新：v1.2（2026-09-14）累计收益新增「是否含当日收益」显式口径（`profit_caliber`），完整定义见 `docs/fund-position-caliber-plan.md`
- 编制日期：2026-09-13
...
> 2026-09-14 口径更新：累计收益新增「是否含当日收益」显式口径（`profit_caliber`），本节按**含当日收益**（默认口径）描述；
> 不含当日收益的折算与降级约定见 `docs/fund-position-caliber-plan.md`。

录入：当前持有金额 `amount`、当前累计收益 `profit`（可为负）、累计收益口径 `profit_caliber`。

...
- P3 新增「持有基金」面板（养基宝式，方案见 `docs/fund-positions-plan.md`，2026-09-13）：手动录入基金代码、当前持有金额与当前累计收益（与个股持仓面板同款极简录入），复用盘中行情与最新官方净值推导当日收益与持仓占比；存储为 `fund_positions` 表 + `.data/fund-positions.json` 回退，接口 `/api/fund-positions`。
- 口径约定：推算本金 = 持有金额 − 累计收益；当日收益 = 持有金额 − 持有金额 /（1 + 当日涨跌幅）；上一交易日累计收益 = 累计收益 − 当日收益，天然满足「上一交易日累计收益 + 当日实时收益 = 累计收益」。当日涨跌幅不可用时对应字段返回 null，而持有金额与累计收益照常展示。
- 后续可选：PostgreSQL 迁移落地后的仓储分支验证、分红/申赎导致的份额变动口径、与个股持仓共用组合总览。

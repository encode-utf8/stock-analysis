...
## 8. 补充修复：基金日报「数据不可用」噪声（2026-09-13）

### 8.1 现象

基金日报 `missing` 为空，但正文出现 4-5 处「本日该数据不可用」，涉及：基金份额变化、折溢价率、跟踪误差、北向资金、两融余额，以及「各板块成分公司数量为 0、不可用」。

### 8.2 根因

- 提示词只写了「数据缺失时必须写出不可用」，没有限定「只有 `missing` 中列出的才算缺失」，模型于是把契约之外的指标一律写成「不可用」。
- 历史行业板块把成分公司数写成 `0`，模型读成「成分公司数为 0 / 不可用」，属于口径表达错误（口径不提供 ≠ 0 家）。

### 8.3 修复

- 提示词硬性要求 2 改写：只有「缺失数据项」列出的内容才允许写「本日该数据不可用」，契约外指标（份额变化、折溢价率、跟踪误差、北向资金、两融余额、板块成分公司数等）一律不得提及。
- `buildDailyReportMessages` 的 user 提示新增「数据口径说明」，并在 `missing` 为空时显式声明「正文中不得出现『不可用』『数据缺失』」。
- 口径说明覆盖三点：历史板块不提供成分公司数与领涨股（null 属口径限制）；板块口径涨跌家数必须写明口径；市场活跃度不得换算成涨跌家数占比。
- 侧车历史板块 `companies` 由 `0` 改为 `null`，`MarketSectorItem.companies` 同步允许 `null`。

### 8.4 结果（2026-09-13 实测）

- `fund/2026-09-11`、`stock/2026-09-11` 强制重新生成：`missing=[]`，「不可用」0 次、「数据缺失」0 次、字段名泄漏 0 次。
- 基金日报正文示例：「市场活跃度指标为 11.57%，该指标与涨跌家数来自不同统计口径，不宜直接换算成涨跌占比」。
- 新增单测 2 例（缺失为空时的禁令、历史口径说明），全套 202 例通过。
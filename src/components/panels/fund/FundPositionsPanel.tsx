...
export function FundPositionsPanel() {
  ...
  return (
    <section className="rounded-xl border bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            只录入基金代码、当前持有金额与当前累计收益；当日实时收益按盘中涨跌幅从持有金额推导，
            上一交易日累计收益 = 当前累计收益 − 当日实时收益。
          </p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="mt-4 grid gap-3 rounded-lg border bg-slate-50/60 p-3 lg:grid-cols-5">
      ...
      {summary ? (
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <MetricCard
            label="当日收益合计"
            value={formatMoney(summary.total_day_profit)}
                ...
                <tr key={position.id} className="border-b align-top">
                  ...
                  <td className={"py-2 pr-3 " + returnTone(valuation.total_profit)}>
                    {editing ? (
                      ...
                      <>
                        <div>{formatMoney(valuation.total_profit)}</div>
                        <div className="text-xs text-muted-foreground">
                          昨日累计 {formatMoney(valuation.prev_total_profit)}
                        </div>
                      </>
                    )}
                  </td>
                  ...
                  <td className="py-2 pr-3">
                    <div className="flex items-center gap-2">
                      <span className="w-14 shrink-0">{valuation.weight_pct.toFixed(2)}%</span>
                      <span className="h-1.5 w-16 shrink-0 overflow-hidden rounded bg-slate-100">
                        <span
                          className="block h-full rounded bg-primary/70"
                          style={{ width: `${Math.min(Math.max(valuation.weight_pct, 0), 100)}%` }}
                        />
                      </span>
                    </div>
                  </td>
      ...
      {summary && holdings.length > 0 ? (
        <p className="mt-3 text-xs text-muted-foreground">
          ...
          累计收益 {formatMoney(summary.total_profit)}（{formatPercent(summary.total_profit_pct)}）· 当日收益{" "}
          {formatMoney(summary.total_day_profit)}
        </p>
      ) : null}
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        当前持有金额与累计收益为手动录入；当日收益按「持有金额 − 持有金额 /（1 + 当日涨跌幅）」估算，取不到盘中行情时该列显示为空。
      </p>
      <p className="mt-1 text-xs text-muted-foreground">
